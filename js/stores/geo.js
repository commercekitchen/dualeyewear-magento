 ...
 jQuery(document).ready(function(){
    $jqr.fn.quickPager = function(options) {
        ...
        var options = $jqr.extend(defaults, options);
        var total=0;
        ...
        return this.each(function() {
            ...
            if (total==0) {
                $jqr("div.toolbar label strong #inform").html('No Items, Please change filter');
            }  
            else {
                first=options.pageSize*(options.currentPage-1)+1;
                last=options.pageSize*(options.currentPage);
                if (last>total) {last=total;}
                $jqr("div.toolbar label strong #inform").html(first+' - '+last+' OF '+total+' | ');
            }    
            ...
            selector.children(".page_it").each(function(i){ 
                ...
                $jqr(this).addClass("simplePagerPage"+pageCounter);
                }
                else {
                    $jqr(this).addClass("simplePagerPage"+(pageCounter+1));
                    pageCounter ++;
                }    
            ...
            if(!options.holder) {
                ...
                case "before":
                    ...
                    prodShow = $jqr('#product_li > li:visible');
                    ...
                    $jqr("div.toolbar label strong #inform").html(first+' - '+last+' OF '+total+' | ');
                    $jqr("div.toolbar label strong #pagination").html(pageNav);
                ...
                $jqr(options.holder).append(pageNav);
            }
            
            //pager navigation behaviour
            $jqr(".simplePagerNav a").live('click', function() {

                //grab the REL attribute 
                var clickedLink = $jqr(this).attr("rel");
                ...
                if(options.holder) {
                    $jqr(this).parent("li").parent("ul").parent(options.holder).find("li.currentPage").removeClass("currentPage");
                    $jqr(this).parent("li").parent("ul").parent(options.holder).find("a[rel='"+clickedLink+"']").parent("li").addClass("currentPage");
                }
                else {
                    ...
                    $jqr('.next').html(pageNext);
                    $jqr('.back').html(pagePrevios);
                    
                    
                    //remove current current (!) page
                    $jqr(".simplePagerNav").find("li.currentPage").removeClass("currentPage");
                    //Add current page highlighting
                    $jqr(".simplePagerNav").find("a[rel='"+clickedLink+"']").parent("li").addClass("currentPage");
                }
