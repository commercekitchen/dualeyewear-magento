...
class Belvg_Stores_Model_State extends Mage_Core_Model_Abstract{
    ...
    */
	public function saveStore($id,$data,$supportedProducts)
    {
        ...
        if (($data['lat']=="") || ($data['lng']=="") || (isset($data['relatlng']))) {
            /* New code start*/
			if($data['zip_code']){
				$new_url = $data['zip_code'];
			}else{
				$new_url = urlencode($data['country']." ".$data['state']." ".$data['city']." ".$data['address']." ".$data['zip_code']);//exit;
			}
  ...
  // print_r($response);
   $geometry = $response['results'][0]['geometry'];
    ...
    $aDBInfo['lng']= $geometry['location']['lat'];
     $aDBInfo['lat']  = $geometry['location']['lng'];
