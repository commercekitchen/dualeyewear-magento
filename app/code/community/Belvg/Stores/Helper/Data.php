...
class Belvg_Stores_Helper_Data extends Mage_Core_Helper_Abstract{
    ...
    */
    public function getLatLng($row,$flag=0)
    {
        ...
        $eng_addr = "";
       /* $url="http://maps.google.com/maps/geo?q=".$new_url."&key="."NgHyGqERMyShY7L9KzyXkjaN"."&sensor=false&hl=en";
        ...
        /*new code*/
         $url= "http://maps.googleapis.com/maps/api/geocode/json?address=".$new_url."&sensor=false";// exit;
     
         $ch = curl_init();
         curl_setopt($ch, CURLOPT_URL, $url);
         curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
         $lat_lng = json_decode(curl_exec($ch), true);
       
        $geometry = $lat_lng['results'][0]['geometry'];
        $result['lat'] =$geometry['location']['lng'] ;
        $result['lng'] = $geometry['location']['lat'];
        $result['eng_addr'] = $row;
        //new code end
